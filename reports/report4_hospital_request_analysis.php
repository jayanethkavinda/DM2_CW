<?php
// ==============================================================================
// 🏥 BUSINESS REPORT 4: HOSPITAL REQUISITION & DISTRIBUTION EFFICIENCY
// Invokes PL/SQL Stored Procedure: rpt_hospital_request_analysis(:p_stat, :p_curs)
// ==============================================================================
...
try {
    $stat_param = !empty($status_filter) ? $status_filter : null;
    $curs = oci_new_cursor($conn);
    $stmt = oci_parse($conn, "BEGIN rpt_hospital_request_analysis(:p_stat, :p_curs); END;");
    oci_bind_by_name($stmt, ":p_stat", $stat_param);
    oci_bind_by_name($stmt, ":p_curs", $curs, -1, OCI_B_CURSOR);

    if (@oci_execute($stmt) && @oci_execute($curs)) {
        while ($r = oci_fetch_assoc($curs)) {
            $rows[] = $r;
            $total_units_needed += intval($r['UNITS_NEEDED'] ?? 0);
            if (strtoupper($r['STATUS']) === 'APPROVED') {
                $approved_count++;
            } elseif (strtoupper($r['STATUS']) === 'PENDING') {
                $pending_count++;
            }
        }
    } else {
...
<head>
    ...
    <title>Report 4: Hospital Blood Requisitions & Distribution</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .title-badge { display: inline-block; background: #e0f2fe; color: #0284c7; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; margin-bottom: 8px; }
...
<div class="container">
    ...
    <div class="header">
        <div>
            <span class="title-badge">PL/SQL BUSINESS REPORT #4</span>
            <h1>Hospital Requisition & Distribution Efficiency Report</h1>
            <p class="subtitle">Tracks hospital blood unit demands, submission timeline, and fulfillment approval status.</p>
        </div>
        <div class="nav-btns">
            <span class="plsql-badge">PROCEDURE: rpt_hospital_request_analysis</span>
    ...
    <div class="kpi-grid">
        <div class="kpi-card">
            ...
            <div>
                <div class="kpi-value"><?php echo $total_units_needed; ?> Units</div>
                <div class="kpi-label">Total Units Requested</div>
            </div>
        </div>
        <div class="kpi-card">
            <div class="kpi-icon" style="background:#dcfce7; color:#16a34a;">&#9989;</div>
            <div>
                <div class="kpi-value"><?php echo $approved_count; ?> Requests</div>
                <div class="kpi-label">Approved & Dispatched</div>
            </div>
        </div>
        <div class="kpi-card">
            <div class="kpi-icon" style="background:#fef3c7; color:#d97706;">&#9203;</div>
            <div>
                <div class="kpi-value"><?php echo $pending_count; ?> Requests</div>
    ...
    <div class="filter-bar">
        <form method="GET" class="filter-form">
            <label for="status" style="font-size:13px; font-weight:600; color:#475569;">Filter by Request Status:</label>
            <select name="status" id="status">
                <option value="">-- All Request Statuses --</option>
                <option value="Approved" <?php echo (strtoupper($status_filter) === 'APPROVED') ? 'selected' : ''; ?>>Approved</option>
                <option value="Pending" <?php echo (strtoupper($status_filter) === 'PENDING') ? 'selected' : ''; ?>>Pending</option>
                <option value="Rejected" <?php echo (strtoupper($status_filter) === 'REJECTED') ? 'selected' : ''; ?>>Rejected</option>
            </select>
            <button type="submit" class="btn btn-primary">&#128269; Filter Requests</button>
            <?php if(!empty($status_filter)): ?>
                <a href="report4_hospital_request_analysis.php" class="btn btn-outline">&#8635; Reset Filter</a>
            <?php endif; ?>
        </form>
    </div>
                ...
                <?php if(empty($rows)): ?>
                    <tr>
                        <td colspan="6" style="text-align:center; padding:40px; color:#94a3b8;">
                            No hospital requisition records match the selected status filter.
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach($rows as $r): ?>
                        <tr>
                            ...
                            <td><strong style="color:#b71c1c; font-size:15px;"><?php echo $r['UNITS_NEEDED']; ?></strong> Units</td>
                            <td><?php echo htmlspecialchars($r['REQ_DATE']); ?></td>
                            <td>
                                <span class="badge-status status-<?php echo htmlspecialchars($r['STATUS_BADGE']); ?>">
